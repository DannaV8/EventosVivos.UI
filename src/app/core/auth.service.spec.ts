...
describe('AuthService', () => {
  ...
  it('login stores token and updates signals', () => {
    ...
    const req = http.expectOne('https://localhost:63801/api/auth/login');
  ...
  it('login with admin token: isAdmin() is true', () => {
    ...
    http.expectOne('https://localhost:63801/api/auth/login').flush({ token: TOKEN_ADMIN });
  ...
  it('logout clears token from signal and localStorage', () => {
    service.login('user@test.com', '12345678').subscribe();
    http.expectOne('https://localhost:63801/api/auth/login').flush({ token: TOKEN_USER });
  ...
  it('register calls POST /auth/register without sending role', () => {
    ...
    const req = http.expectOne('https://localhost:63801/api/auth/register');
    expect(req.request.method).toBe('POST');
